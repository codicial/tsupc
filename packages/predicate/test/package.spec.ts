...
describe("@tsupc/predicate packaging", () => {
  it("supports ESM self-imports", () => {
    run(process.execPath, [
      "--input-type=module",
      "--eval",
      [
        "import * as test from '@tsupc/predicate';",
  ...
  it("supports CommonJS self-requires", () => {
    run(process.execPath, [
      "--eval",
      [
        "const test = require('@tsupc/predicate');",
  ...
  it("resolves declaration files for TypeScript consumers", () => {
    ...
    try {
      writeFileSync(
        ...
        ["import * as test from '@tsupc/predicate';", "void test;"].join("\n"),
      );
